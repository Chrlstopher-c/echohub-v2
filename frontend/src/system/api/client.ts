/*
 * Accès HTTP du domaine `system`.
 *
 * Le client vit DANS le domaine : `shared/` n'expose pas de client API, et un domaine ne dépend
 * pas d'un module qui n'existe pas. La trentaine de lignes dupliquée avec le domaine `models` est
 * assumée — c'est du code accidentellement semblable, pas une règle métier partagée.
 */

/** Racine servie par nginx ; le backend porte lui-même le reste du chemin. */
...
export const journal = {
  ...
  erreur(contexte: string, cause: unknown): void {
    console.error(`[system] ${contexte}`, cause);
  },
};
...
/** URL absolue d'un flux SSE : `EventSource` n'accepte pas de chemin relatif à une base. */
export function urlFlux(chemin: string): string {
  return `${RACINE_API}${chemin}`;
}
